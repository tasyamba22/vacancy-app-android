...
class AddEditVacancyViewModel @Inject constructor(
    ...
    fun save(existingId: Int?) = viewModelScope.launch {
        ...
        if (state.title.isBlank() || state.company.isBlank()) {
            _uiState.update {
                it.copy(error = getApplication<Application>().getString(R.string.addedit_error_required_fields))
            }
            return@launch
        }
        ...
        try {
            ...
            if (existingId != null) {
                vacancyRepository.updateVacancy(
                    token, existingId, state.title, state.company,
                    state.salary.ifBlank { null }, state.location.ifBlank { null },
                    state.description.ifBlank { null }
                )
            } else {
                vacancyRepository.createVacancy(
                    token, state.title, state.company,
                    state.salary.ifBlank { null }, state.location.ifBlank { null },
                    state.description.ifBlank { null }
                )
            }
